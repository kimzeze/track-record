...
```
PR closed + merged → AI judge → AI build → vault repo push → Slack 알림
```
...
```
┌─────────────────────────────────────────────┐         ┌──────────────────────────┐
│  대상 레포 (frontend-aptimizer 등)            │         │  vault 레포               │
│                                               │         │  (track-record-vault)    │
│  .github/workflows/track-record.yml          │         │                          │
│  → 13줄 YAML, 시크릿 전달만 담당               │         │  kimzeze/                │
└──────────────────┬──────────────────────────┘         │  ├── README.md           │
                   │ workflow_call                       │  ├── frontend-aptimizer  │
                   ▼                                     │  │   .md                 │
┌─────────────────────────────────────────────┐  push   │  └── aptifit-frontend    │
│  track-record (이 레포)                       │ ─────► │      .md                 │
│                                               │         │                          │
│  .github/workflows/curate.yml (Executor)     │         │  alice/                  │
│  → 큐레이터 로직 전체 실행                     │         │  └── ...                 │
│  → 프롬프트, 모델, 설정 모두 중앙 관리          │         │                          │
└─────────────────────────────────────────────┘         └──────────────────────────┘
```
...
```
                    PR Context (diff + 메타)
                            ...
                            ▼
                   ┌────────────────────┐
                   │  ① Threshold Judge │  ← 가벼운 모델 (haiku-4.5)
                   │  PASS / SKIP 판정    │     ~$0.005/PR
                   └────────┬───────────┘
                            │
                  ┌─────────┴─────────┐
                  │ SKIP              │ PASS
                  ▼                   ▼
             ┌────────┐    ┌──────────────────┐
             │ Slack  │    │ ② Skill Matcher  │  ← 변경 파일 → 베스트 프랙티스 매칭
             │ (skip) │    └────────┬─────────┘
             └────────┘             │
                                    ▼
                          ┌──────────────────┐
                          │ ③ Entry Builder  │  ← 강한 모델 (sonnet-4.6)
                          │ 4문장 STAR 작성   │     ~$0.05/PR
                          └────────┬─────────┘
                                   │
                                   ▼
                          ┌──────────────────┐
                          │ ④ Entry Merger   │  ← 강한 모델 (sonnet-4.6)
                          │ 기존과 통합 판단  │     ~$0.03/PR
                          └────────┬─────────┘
                                   │
                          ┌────────┴────────┐
